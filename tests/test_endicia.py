...
class BaseTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.GenerateLabel = POOL.get('generate.shipping.label', type="wizard")
...
class TestUSPSEndicia(BaseTestCase):
    ...
    def test_0016_generate_endicia_flat_label_using_wizard(self):
        ...
        with Transaction().start(DB_NAME, USER, context=CONTEXT):
            ...
            with Transaction().set_context(
                company=self.company.id, active_ids=[shipment]
            ):
                # Call method to generate labels.
                ...
                self.assertEqual(result['cost'], Decimal('2.59'))
                self.assertEqual(
                    result['cost_currency'], shipment.cost_currency.id
                )

                generate_label.start.shipment = result['shipment']
                generate_label.start.carrier = result['carrier']
                generate_label.start.cost = result['cost']
                generate_label.start.cost_currency = result['cost_currency']
    ...
    def test_0016_generate_endicia_label_using_wizard_without_carrier(self):
        """
        Test case to generate Endicia labels using wizard without carrier
        """
        with Transaction().start(DB_NAME, USER, context=CONTEXT):
            ...
            with Transaction().set_context(
                ...
                self.assertEqual(result['shipment'], shipment.id)
                self.assertFalse(result.get('carrier'))

                generate_label.start.shipment = result['shipment']
                generate_label.start.carrier = self.carrier.id
                generate_label.start.cost = Decimal('10')
                generate_label.start.cost_currency = self.currency.id
                ...
                generate_label.endicia_config.endicia_mailclass = \
                    result['endicia_mailclass']
                generate_label.endicia_config.endicia_mailpiece_shape = 'Card'
                generate_label.endicia_config.endicia_label_subtype = \
                    result['endicia_label_subtype']
                generate_label.endicia_config.endicia_integrated_form_type = \
                    result['endicia_integrated_form_type']
                generate_label.endicia_config.endicia_package_type = 'Gift'
                generate_label.endicia_config.endicia_include_postage = \
                    result['endicia_include_postage']
                generate_label.endicia_config.endicia_shipment_bag = None
            ...
            self.assertEqual(shipment.cost, Decimal('2.59'))
            self.assertEqual(shipment.cost_currency, self.currency)
            self.assertEqual(shipment.endicia_mailpiece_shape, 'Card')
            self.assertTrue(
                self.IrAttachment.search([
                    ('resource', '=', 'stock.shipment.out,%s' % shipment.id)
                ], count=True) > 0
